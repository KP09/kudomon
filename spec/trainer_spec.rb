...
describe Trainer do
  ...
  # Catching kudomon
  describe '#catch' do
    let(:nearby) { mid_grid + NEARBY / 2 }
    let(:too_far) { mid_grid + NEARBY }

    # Selects a random species to be in range
    let!(:in_range_species) { KUDOMON.keys.sample }

    # Selects a random species to be out of range
    # and makes sure it is not the same as the species in range
    let!(:all_other_species) { KUDOMON.reject { |k, _v| k == in_range_species } }
